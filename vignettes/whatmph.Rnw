...
Let's have a look at a generated dataset which simulates an unemployment register.
<<data>>=
library(durmod)
data(durdata)
head(durdata,15)
...
However, there is a mixture part also, designed to account for unobserved individual
...
be created by the busy CRAN repository, we limit ourselves to 6
iterations, i.e. no more than 6 masspoints in the distribution. For
the same reason we also limit to 2 cpus, or threads, in the
...
indexes into the risksets argument. 
...
If we had limited to 5 masspoints, the \code{alpha} estimate would be negative, i.e. 
the labour market programme has a negative effect on the hazard for getting a job.
The reader may run the example with a maximum of 15 iterations to obtain a positive effect.
...
should use the \code{timing="interval"} argument in \code{mphcrm}.  This model is
notoriously difficult to estimate because the likelihood involves ``doubly exponential'' 
expressions of the type \(1-\exp(-\exp(x))\), which are numerically challenging.

If the hazards are small, 1-2\% per time unit, the difference between the interval and exact model
are quite small, so one may opt for using the exact model instead.
...
There are many control parameters. Here are some one may want to tinker with.
\begin{itemize}
...
\item{\code{cluster.}} A cluster from package \code{parallel} or \code{snow}. The dataset will be
