...
In our dataset, we we have an observation period of 80, and the individuals enter
...
It can be remade with the commands:
<<eval=FALSE>>=
library(durmod)
data(durdata)
...
Their covariance was 0.00656. We see that our estimates are in the vicinity.
