...
Then we create a set of control parameters, since this is a demonstration dataset with only
a few thousand observations, we limit
ourselves to 5 iterations, i.e. no more than 5 masspoints in the distribution. Since
this vignette is to be created by the busy CRAN repository, we also limit ourselves to using
...
indexes into the risksets argument. Note that the names of the list \code{risksets} are
...
The limit to 5 masspoints has the consequence that the \code{alpha} estimate is negative, i.e. 
the labour market programme has a negative effect on the hazard for getting a job.
The reader may run the example with a maximum of 15 iterations, and get a positive effect.
\end{document}
